...
const DealOfTheDaySection = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="pb-10 bg-green-700"><div class="container mx-auto"><div class="px-4 pt-16 text-green-50 md:px-20"><div class="mb-8 text-3xl font-bold md:text-4xl md:mb-4" data-svelte-h="svelte-15cejys">Deals of the day</div> <div class="flex gap-4 justify-start mb-6 md:mb-14"><p class="text-lg font-normal text-green-300" data-svelte-h="svelte-ifqhiy">Time Left</p> <p class="text-lg font-semibold text-green-50 md:w-fit">${escape(formattedTime)}</p></div> <div class="overflow-x-scroll pb-2 flex gap-2">${loading ? `<p data-svelte-h="svelte-qdsr2u">Loading...</p>` : `${productsList ? `${each(productsList, (product) => {
    return `<div class="bg-green-600 p-4"><div class="p-2 bg-green-300 shadow-lg"><img class="w-[208px] h-[250px]"${add_attribute("src", product.photo, 0)}${add_attribute("alt", product.product, 0)}></div> <div class="p-2 mt-8 bg-green-300/20 w-48"><p class="mb-1 text-xl font-normal text-green-50 whitespace-nowrap">${escape(product.product)}</p> <div class="flex justify-between items-center"><p class="text-sm font-normal"><span class="text-green-900">Rs ${escape(product.price)}</span></p> <button class="px-4 py-1 text-sm text-white bg-green-800 shadow-sm hover:bg-green-900 active:bg-green-800" data-svelte-h="svelte-1gqkzz4">Add to cart</button> </div></div> </div>`;
  })}` : ``}`}</div></div></div></div>`;
});
const HeroSection = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<div class="bg-green-300" data-svelte-h="svelte-gox1z4"><div class="container mx-auto"><div class="flex flex-col pb-10 md:pb-20 place-content-end h-[90vh] max-h-[1000px] px-4 md:px-20 max-w-screen-xl"><div><h1 class="mb-10 min-w-full text-7xl font-bold lg:text-8xl">Lovely <span class="text-green-900">plants</span>,<br> for lovely
					<span class="text-green-900">people</span></h1> <p class="mb-10 text-base font-semibold text-gray-600 md:mb-14 md:w-1/2 lg:w-1/3">Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis optio cupiditate rem
